...
restart_services() {
    ...
    systemctl enable --now srvpanel-packages.timer >/dev/null 2>&1 || true
    ...
    systemctl enable --now srvpanel-backups.timer >/dev/null 2>&1 || true
    systemctl enable --now srvpanel-traffic.timer >/dev/null 2>&1 || true
