...
const props = defineProps<{
  ...
  /*
   * Hier standen bis August 2026 zwei weitere Zahlen: `withdrawn` und
   * `targets`. Ein zurückgebautes Abonnement blieb damals als Zeile liegen,
   * hielt den Plan über seinen Fremdschlüssel fest und musste beim Löschen
   * irgendwohin — der Betreiber wählte das Ziel neben dem Knopf.
   *
   * Seit docs/35 gibt es diese Zeilen nicht mehr; der Systembenutzer ist in
   * `system_users` reserviert. Am Plan hängt nichts Unsichtbares mehr, und die
   * Auswahl hatte keinen Anlass mehr. Das steht hier, damit der Rückbau später
   * nicht wie eine vergessene Funktion aussieht.
   */
...
/*
 * Löschbar ist ein Plan, an dem kein Abonnement mehr hängt.
 *
 * Gefragt wird dieselbe Frage wie in `PlanController::destroy()`. Vorher hing
 * der Knopf an gar nichts, und die Prüfung dahinter zählte anders als der
 * Fremdschlüssel; ein Plan mit einem zurückgebauten Abonnement sah damit
 * löschbar aus und antwortete mit einem 500er. Die Zahl kommt weiter vom
 * Server und wird nicht aus dem abgeleitet, was die Seite sonst noch weiss.
 */
const removable = computed(() => props.subscriptions === 0)
...
<template>
  ...
  >
    ...
    <!--
      Hier stand ein Hinweis auf zurückgebaute Abonnements und daneben die
      Auswahl, an welchen Plan sie übergehen — beides ist mit docs/35
      weggefallen, weil es die Zeilen nicht mehr gibt. Der Löschknopf hängt
      seitdem nur noch an einer Zahl.
    -->
