...
> **Beim Bauen entschieden (2. September):** `/run/sshd` wird **nicht**
> angelegt. `SftpAccess::ensureRuntime()` tut das, weil es schreiben darf; die
> Diagnose meldet `unreachable` mit dem Grund. Das ist §5.1 wörtlich genommen —
> und der eine Fall, in dem ein `mkdir` verlockend gewesen wäre.
...
| `ManagedBlockIntegrityTest` | **gebaut** — der lesende Blick meldet `BEGIN` ohne `END`, `END` ohne `BEGIN` und den doppelten Block an den neun Formen aus M14, **und er zählt wie der Schreiber**: Wo `without()` wirft, sagt `inspect()` `begin_without_end`, nirgends sonst. Die fremde Zeile hält er ausdrücklich **nicht** — sie braucht neben dem Sollzustand auch die Zeilen aus der Datei und ist Schritt 5b (`ManagedBlockDriftTest`) |
...
| `ManagedBlockDriftTest` | **gebaut** — die fremde Zeile aus M16 wird von unserer unterschieden, **in beide Richtungen**; ein Befund je Art und nicht je Zeile; kein Block und nichts im Bestand ist kein Befund; der Sollzustand kommt aus der Vorlage und wird nicht nachgebaut; und genau **eine** Prüfung schreibt `block.integrity` |
...
- ~~**§3 C Frage 3 — der Vergleich mit dem Sollzustand.**~~ **Gebaut am
  2. September 2026 als Schritt 5b.** Er fiel zwischen zwei Schritte, weil er
  beides braucht: die Zeilen aus der Datei, die nur der Agent lesen kann
  (`pg_hba.conf` ist `0640 postgres:postgres`), und den Sollzustand, den nur
  das Panel kennt. Der Agent gibt seine Zeilen jetzt heraus, das Panel
  vergleicht mit `RemoteAccess::orphans()`/`::missing()` und
  `SshdConfig::lines()`, und `DiagnoseSeamTest::SPRACHLOS` ist leer.
