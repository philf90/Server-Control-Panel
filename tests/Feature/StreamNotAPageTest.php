 ...
 * `docs/39` Punkt 3 ist genau daran hängengeblieben.
 *
 ...
 * > gebrochen wird.**
 *
 * ## Warum das hier steht und nicht mehr in `PreviousUrlTest`
 *
 * **Weil es dort zwei Tage lang gar nicht mehr stand.** Beide Fälle waren
 * ursprünglich Teil von `PreviousUrlTest`. Am 11. August 2026 hat der Fund zu
 * {@see RememberPageUrl} diese Datei übernommen — gleicher
 * Name, gleiches Thema, **anderer Gegenstand** —, und die zwei Fälle zu
 * {@see KeepPreviousUrl} sind dabei ersatzlos verschwunden. Die Mittelschicht
 * blieb, ihr Eintrag in `routes/web.php` blieb, der Wächter darüber war fort.
 *
 * Gemerkt hat es **nur `tests/waechter-brechen.sh`**: Seine beiden Eingriffe
 * liefen ins Leere und meldeten „kein Test". Kein anderes Werkzeug dieses
 * Projekts sieht das — `GuardReachTest` prüft Klassennamen in Kommentaren, und
 * die Klasse gab es ja weiterhin.
 *
 * > **Zwei Regeln in einer Datei, die nach der einen heisst, verlieren die
 * > andere beim nächsten Umbau.**
 *
 * Deshalb steht das Gegenstück jetzt in einer eigenen Datei, die nach *seiner*
 * Regel heisst. Und deshalb prüft `BreakScriptTest` seit demselben Tag, dass
 * jeder Eingriff einen Test **mit diesem Namen** nennt — nicht nur eine Klasse,
 * die es gibt.
 */
final class StreamNotAPageTest extends TestCase
{
    /**
